...
def Euler(f,xn,dt,t):
    ...
    return xn + dt*f(xn,t)     # funcion dependiente de x_n para obtener x_n+1
...
def RK4(f,xn,dt,t):
    ...
    return xn + (dt/6)*(f(xn,t) + 2*f(xn + f(xn,t)/2, t+dt/2) + 2*f(xn + f(xn + f(xn,t)/2, t+dt/2)/2, t+dt/2) + f(xn + f(xn + f(xn + f(xn,t)/2, t+dt/2)/2, t+dt/2), t+dt))
