...
from Library.Convergencia_y_Desviacion_ODEs import Richardson, Convergence_rate
from Library.Problema_Cauchy import Cauchy


""" Milestone 3: Discursión del error y la velocidad de covergencia """

""" Estudio de distintos propagadores (Euler explícito e implícito, 
    Crank Nicolson, RK, ...)                                                """ 
...
E_Euler   = Richardson(Euler,1,Kepler,xo,t1,t2); print("Error de Euler calculado\n")
E_Euler_i = Richardson(Euler_implicito,1,Kepler,xo,t1,t2); print("Error de Euler Implicito calculado\n")
E_CN      = Richardson(CN,2,Kepler,xo,t1,t2); print("Error de Crank-Nicolson calculado\n")
E_RK4     = Richardson(RK4,4,Kepler,xo,t1,t2); print("Error de Runge Kutta calculado\n")
print("Errores de Richardson obtenidos\n")

# Velocidad de convergencia de los esquemas temporales
log_NE, log_EE = Convergence_rate(Euler,1,Kepler,xo,t1,imax=10); print("Convergencia de Euler determinada\n")
log_NEi, log_EEi = Convergence_rate(Euler_implicito,1,Kepler,xo,t1,imax=10); print("Convergencia de Euler Implicito determinada\n")
log_NCN, log_ECN = Convergence_rate(CN,2,Kepler,xo,t1,imax=10); print("Convergencia de Crank-Nicolson determinada\n")
log_NRK, log_ERK = Convergence_rate(RK4,4,Kepler,xo,t1,imax=10); print("Convergencia de Runge Kutta determinada\n")
print("Velocidades de converdencia realizadas\n")

# Euler
r_E1 = Cauchy(Euler,Kepler,xo,t1)
r_E2 = Cauchy(Euler,Kepler,xo,t2)

# Euler implicito
r_Ei1 = Cauchy(Euler_implicito,Kepler,xo,t1)
r_Ei2 = Cauchy(Euler_implicito,Kepler,xo,t2)

# CN
r_CN1 = Cauchy(CN,Kepler,xo,t1)
r_CN2 = Cauchy(CN,Kepler,xo,t2)

# RK4
r_RK1 = Cauchy(RK4,Kepler,xo,t1)
r_RK2 = Cauchy(RK4,Kepler,xo,t2)
...
Bar(["Euler","Euler Implicito","Crank-Nicolson","Runge-Kutta 4"],
    [(max(E_Euler[1,:])**2 + max(E_Euler[2,:])**2)**0.5,
     (max(E_Euler_i[1,:])**2 + max(E_Euler_i[2,:])**2)**0.5,
     (max(E_CN[1,:])**2 + max(E_CN[2,:])**2)**0.5,
     (max(E_RK4[1,:])**2 + max(E_RK4[2,:])**2)**0.5], Title="Error máx cometido en la posición")

Bar(["Euler","Euler Implicito","Crank-Nicolson","Runge-Kutta 4"],
    [(sum(E_Euler[1,:])**2 + sum(E_Euler[2,:])**2)**0.5,
     (sum(E_Euler_i[1,:])**2 + sum(E_Euler_i[2,:])**2)**0.5,
     (sum(E_CN[1,:])**2 + sum(E_CN[2,:])**2)**0.5,
     (sum(E_RK4[1,:])**2 + sum(E_RK4[2,:])**2)**0.5], Title="Error acumulado en la posición")
